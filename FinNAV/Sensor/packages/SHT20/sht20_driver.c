...
void SHT2x_SoftReset(void){
	...
	HAL_I2C_Master_Transmit(_sht2x_ui2c, SHT2x_I2C_ADDR << 1, &cmd[0], 2, SHT2x_TIMEOUT);
}
...
{
	*temp = (uint16_t)((uint32_t)(SHT2x_GetTemperature(1) * 100.0f));
	*rh = (uint16_t)((uint32_t)(SHT2x_GetRelativeHumidity(1) * 100.0f));
}
